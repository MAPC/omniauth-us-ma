...
describe OmniAuth::Strategies::UsMa do
  let(:request) { double('Request', :params => {}, :cookies => {}, :env => {}) }

  subject do
    args = ['example_appid', 'example_secret', @options || {}].compact
    OmniAuth::Strategies::UsMa.new(*args).tap do |strategy|
  ...
  describe 'client options' do
    it 'should have correct name' do
      expect(subject.options.name).to eq('us_ma')
    end

    it 'should have correct site' do
      expect(subject.options.client_options.site).to eq('https://api.massit.gov')
    end
